...
# ==========================================
# ISOLAMENTO MULTI-TENANT E VALIDAÇÃO REAL-TIME
# ==========================================
class TenantPermissionsMixin:
    ...
    # Oculta botões globalmente se a assinatura cair
...
class NarrativaAdmin(TenantPermissionsMixin, admin.ModelAdmin):
    ...
    # Validação do Limite de Narrativas
    def has_add_permission(self, request):
        if request.user.is_superuser: return True
        clinica = self.get_clinica_atualizada(request)
        if clinica:
            if not clinica.assinatura_ativa: return False
            try:
                qtd_atual = Narrativa.objects.filter(clinica=clinica).count()
                if qtd_atual >= clinica.plano_atual.limite_narrativas:
                    return False
            except Exception:
                pass
            ...
            for pf in perfis_a_processar:
                ...
                if pergunta.tipo_resposta == "TEXTO":
                    ...
                    dados_comparativos[pergunta.id]['dados_por_perfil'].append({
                        ...
                        'dados_grafico': {'labels': json.dumps(list(c.keys())), 'data': json.dumps(list(c.values())), 'tipo_grafico': 'pie' if pergunta.tipo_resposta == "UNICA_ESCOLHA" else 'bar'}
                    })
        ...
        if export_format in ['xlsx', 'csv', 'json']:
            ...
            for r in respostas:
                if r.session_key not in dados_exportacao:
                    dados_exportacao[r.session_key] = {'data': r.data_resposta.strftime('%d/%m/%Y %H:%M'), 'respostas': {}}
