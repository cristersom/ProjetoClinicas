...
class PlanosView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Lista de planos com os IDs de preço reais fornecidos
        context['planos'] = [
            {
                'nome': 'Básico',
...
def stripe_webhook(request):
    ...
    # Lógica de ativação após sucesso no pagamento
    if event['type'] == 'checkout.session.completed':
        ...
        if clinica_id:
            try:
                ...
                # Opcional: Atualizar o plano baseado no price_id recebido
                clinica.save()
