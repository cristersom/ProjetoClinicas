...
class CriarContaView(CreateView):
    ...
    def form_valid(self, form):
        ...
        email_sessao = self.request.session.get('email_pagamento')
        plano_id = self.request.session.get('plano_pago_id')

        if email_sessao == user.email:
            if hasattr(user, 'clinica') and user.clinica:
                user.clinica.assinatura_ativa = True
                if plano_id:
                    user.clinica.plano = Plano.objects.filter(id=plano_id).first()
                user.clinica.save()
                if 'email_pagamento' in self.request.session:
                    del self.request.session['email_pagamento']
                if 'plano_pago_id' in self.request.session:
                    del self.request.session['plano_pago_id']
