...
class PlanosView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['planos'] = [
            {'nome': 'Básico', 'preco': '49', 'limite': '5 Jornadas', 'pid': 'price_123_basic'},
            {'nome': 'Profissional', 'preco': '99', 'limite': '20 Jornadas', 'pid': 'price_456_pro'},
            {'nome': 'Avançado', 'preco': '199', 'limite': 'Ilimitado', 'pid': 'price_789_adv'},
            {'nome': 'Enterprise', 'preco': 'Consultar', 'limite': 'Customizado', 'pid': None},
        ]
        return context
...
def criar_checkout_sessao(request, price_id):
    ...
    checkout_session = stripe.checkout.Session.create(
        ...
        cancel_url=request.build_absolute_uri(reverse('narrativa:home')),
...
def stripe_webhook(request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        # Aqui você pode mapear os IDs de preço para limites reais
        clinica.save()
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if clinica:
            ...
            context['kpi_respostas'] = Resposta.objects.filter(pergunta__questionario__cena_associada__narrativa__clinica=clinica).count()
...
class PacienteNarrativas(ListView):
    ...
    model = Narrativa
    def get_queryset(self): return Narrativa.objects.all().order_by('-visualizacoes')
...
def exibir_cena_paciente(request, cena_id):
    ...
    LogVisitaCena.objects.get_or_create(session_key=request.session.session_key, cena_visitada=cena, defaults={'narrativa_associada': cena.narrativa})
...
def responder_questionario(request, questionario_id):
    ...
    if request.method == "POST":
        ...
        return redirect('narrativa:exibir_cena_paciente', cena_id=questionario.cena_destino.id) if questionario.cena_destino else redirect('narrativa:paciente_narrativas')
